...
from evaluation.lpips_loss_yifan import *
...
def build_optimizer_sched(opt, net, log):
    ...
    log.info("[Opt] Built AdamW optimizer {optim_dict=}!")

    if opt.lr_gamma < 1.0:
        sched_dict = {"step_size": opt.lr_step, 'gamma': opt.lr_gamma}
        sched = lr_scheduler.StepLR(optimizer, **sched_dict)
        #log.info(f"[Opt] Built lr step scheduler {sched_dict=}!")
        log.info("[Opt] Built lr step scheduler {sched_dict=}!")
    else:
        sched = None
    ...
    if opt.load:
        ...
        if "optimizer" in checkpoint.keys():
            ...
            log.info("[Opt] Loaded optimizer ckpt {opt.load}!")
        else:
            #log.warning(f"[Opt] Ckpt {opt.load} has no optimizer!")
            log.warning("[Opt] Ckpt {opt.load} has no optimizer!")
        if sched is not None and "sched" in checkpoint.keys() and checkpoint["sched"] is not None:
            ...
            log.info("[Opt] Loaded lr sched ckpt {opt.load}!")
        else:
            #log.warning(f"[Opt] Ckpt {opt.load} has no lr sched!")
            log.warning("[Opt] Ckpt {opt.load} has no lr sched!")
...
class Runner(object):
    def __init__(self, opt, log, save_opt=True):
        ...
        log.info("[Diffusion] Built I2SB diffusion: steps={len(betas)}!")
        ...
        if opt.load:
            ...
            log.info("[Net] Loaded network ckpt: {opt.load}!")
            self.ema.load_state_dict(checkpoint["ema"])
            #log.info(f"[Ema] Loaded ema ckpt: {opt.load}!")
            log.info("[Ema] Loaded ema ckpt: {opt.load}!")
        ...
        self.perceptual_loss = LPIPS().to(opt.device)
        self.perceptual_loss = self.perceptual_loss.eval()
    ...
    def sample_batch(self, opt, loader, corrupt_method):
        if opt.corrupt == "mixture":
            ...
            index, data_batch = next(enumerate(loader))
  
            clean_img = data_batch['B']
            corrupt_img = data_batch['A']
            y = data_batch['label']
            mask = None
    ...
    def ddpm_sampling(self, opt, x1, mask=None, cond=None, clip_denoise=False, nfe=None, log_count=10, verbose=True):
        ...
        self.log.info("[DDPM Sampling] steps={opt.interval}, {nfe=}, {log_steps=}!")
    ...
    def evaluation(self, opt, it, val_loader, corrupt_method):
        ...
        log.info("========== Evaluation started: iter={it} ==========")
        ...
        np.save('Save_result_Yifan/img_clean'+str(it)+'.npy',img_clean)
        np.save('Save_result_Yifan/img_corrupt'+str(it)+'.npy',img_corrupt)
        np.save('Save_result_Yifan/img_recon'+str(it)+'.npy',img_recon)
        
        log_image("image/clean",   img_clean)
        log_image("image/corrupt", img_corrupt)
        log_image("image/recon",   img_recon)    ### [bs, 1, 64, 64]
        ...
        label_doctor = []
        for i in range(450):
            fake_B = copy.deepcopy(img_recon[i,:,:])
            #Fake_B_max = torch.max(fake_B)
            #Fake_B_min = torch.min(fake_B)
            #fake_B = (fake_B-Fake_B_min) / (Fake_B_max-Fake_B_min)
            #fake_B = (fake_B-0.5)/0.5

            real_B = copy.deepcopy(img_clean[i,:,:])
            real_A = copy.deepcopy(img_corrupt[i,:,:])
            
            real_A = real_A[None,:].to(opt.device)
            real_B = real_B[None,:].to(opt.device)
            fake_B = fake_B[None,:].to(opt.device)

            p_loss_all = self.perceptual_loss(real_B, fake_B)
            ...
            out1.append(out_1[0].detach().cpu().numpy())
            out2.append(out_2[0].detach().cpu().numpy())   
            ...
            label_doctor.append(label_cancer)



        
        label_eval = np.array(label_doctor)
        out1_eval = np.array(out1)
        out2_eval = np.array(out2)

        print(label_eval)
        print(out1_eval)
        fpr, tpr, thresholds = metrics.roc_curve(label_eval, np.array(out1_eval),pos_label=1)
        AAAUC1 = metrics.auc(fpr, tpr)
        print('TEST_AUC_val_1',AAAUC1) 

        fpr, tpr, thresholds = metrics.roc_curve(label_eval, np.array(out2_eval),pos_label=1)
        AAAUC2 = metrics.auc(fpr, tpr)
        print('TEST_AUC_val_2',AAAUC2) 
        ...
        log.info("========== Evaluation finished: iter={it} ==========")
